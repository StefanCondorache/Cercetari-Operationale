...
### Key Features:
  ...
  * **Variable Domains**: Natively supports strictly positive (x ≥ 0), strictly negative (x ≤ 0), and free/unrestricted variables (x ∈ ℝ) through automated internal variable substitution.
...
Executes the Primal Simplex Algorithm and returns the result of the optimization.
  ...
  * **Optimal Solution (Dictionary):** If a finite optimal solution is found, it returns a detailed dictionary containing metadata for every variable (decision, slack, surplus, and artificial).
    ```python
    {
      'x1': {'valoare': 20.0, 'tip': 'decizie (>=0)', 'coef_obiectiv': 15.0},
      'x2': {'valoare': -5.0, 'tip': 'decizie (<=0)', 'coef_obiectiv': 10.0},
      'x3': {'valoare': 12.5, 'tip': 'decizie (R)',   'coef_obiectiv': 5.0},
...
Performs three independent mathematical checks to validate the integrity of the computed solution. It returns a `dict` containing the boolean results `[bool, bool, bool]` and detailed mathematical proofs.
  ...
  * **Check 1 (Non-negativity):** `True` if all internal transformed variables in the final solution satisfy x ≥ 0.
  ...
  * **Check 3 (Matrix Consistency):** `True` if multiplying the initial basis (S) by the final state accurately reconstructs the initial constants (b0).
...
### GUI Features:
  ...
  * **Variable Domain Selection:** Interactive dropdowns for every decision variable to define its individual domain (≥ 0, ≤ 0, or free/R).
  ...
  * **Visual Mathematical Proofs:** Renders the algebraic consistency checks (e.g., matrix reconstruction S \* X\_b = b) as beautifully formatted, monospace equations directly in the UI.
...
problem = {
    "coef": np.array([3, 2], dtype=np.float64),            
    ...
    "x": np.array([0, 2], dtype=int),           # 0 for x ≥ 0, 1 for x ≤ 0, 2 for x ∈ ℝ
...
It includes:
  ...
  * **Standard Integrations:** Minimization & Maximization.
  * **Scale Testing:** 10x10 Enterprise resource planning simulations.
  * **Degeneracy & Ties:** Equal Deltas, identical coefficients, and b\_i = 0 cases.
  * **Classic Operations Research Cycles:** Beale's Cycle, Marshall's Cycle, and Bland's rule stress tests.
  * **Edge Cases:** Unrestricted variables, negative domains, Big-M swamping, negative b constraints, and redundant equality rows.
